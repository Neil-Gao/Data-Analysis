...
rm(list=ls())
setwd("/Users/aimhighfly/Documents/StonyBrookUniversity/Spring_semester/EST508/Projects/Lendingclub")
report_date = 201603
load("LC.RData")
LC0 = LC
...
qplot(x = sub_grade, y = avg_fico, data = sub_gradeVSfico, group = 1, geom = "point", size = I(2), col = I("steelblue")) + 
  theme_bw() + 
  geom_hline(yintercept = 688, color = "red") + 
  geom_hline(yintercept = 678, color = "red") + 
...
RejectStatsA = read.csv("RejectStatsA.csv", stringsAsFactors = FALSE, header = T, skip = 1)
...
# 
# 
# LC.test = subset(LC1, select = c(fico_range_high,loan_status))
# LC.test$loan_status = as.factor(LC.test$loan_status)
# 
# LC1$loan_amnt, LC1$funded_amnt, LC1$term, LC1$int_rate, (LC1$installment), LC1$grade, LC1$sub_grade,
# LC1$emp_length, LC1$home_ownership, LC1$annual_inc, LC1$verification_status, LC1$issue_d, LC1$purpose, LC1$addr_state,
# LC1$dti, LC1$delinq_2yrs, LC1$earliest_cr_line, LC1$fico_range_low, LC1$fico_range_high,
# LC1$inq_last_6mths, LC1$mths_since_last_delinq, LC1$mths_since_last_record, LC1$open_acc, LC1$revol_bal,
# LC1$revol_util, LC1$total_acc, LC1$initial_list_status, LC1$collections_12_mths_ex_med
# 
# LC1$loan_status
# 
# str(LC.test)
# LC.test$loan_status[1000,]



#__________________________________________________________________________________________________________________
# Combine reject date A, B, D
RejectStatsA = read.csv("RejectStatsA.csv", stringsAsFactors = FALSE, header = T, skip=1)
RejectStatsB = read.csv("RejectStatsB.csv", stringsAsFactors = FALSE, header = T, skip=1)
RejectStatsD = read.csv("RejectStatsD.csv", stringsAsFactors = FALSE, header = T, skip=1)
RDE= rbind(RejectStatsA, RejectStatsB, RejectStatsD)
RD = tbl_df(RDE)
...
LC.vs = data.frame("Amount" = LC$loan_amnt,
                   "Score" = LC$fico_range_low,
                   "DTI" = as.numeric(LC$dti),
                   "Employment.length" = LC$emp_length,
                   "Status" = factor(1)
)
LC1.vs <- na.omit(LC.vs)
str(LC.vs)
str(LC1.vs)

RD.vs = data.frame("Amount" = RD$Amount.Requested,
                "Score" = RD$Risk_Score,
                "DTI" = as.numeric(strsplit(RD$Debt.To.Income.Ratio, '%')),
                "Employment.length" = as.factor(RD$Employment.Length),
                "Status" = factor(0)
        )
RD1.vs <- na.omit(RD.vs)
str(RD.vs)
str(RD1.vs)

save(LC1.vs, file = "LC.vs.RData")
save(RD1.vs, file = "RD.vs.RData")

#________________________________________________________________________________________________________________
load("LC.vs.RData")
load("RD.vs.RData")
LC.sample = sample_n(LC1.vs, 20000, replace = FALSE)
str(LC.sample)
RD.sample = sample_n(RD1.vs, 20000, replace = FALSE)
str(RD.sample)


VS = rbind.data.frame(LC.sample, RD.sample)
...
set.seed(7)
fit.lda <- train(Status ~., data=VS, method = "lda", metric = metric, trControl = control)
fit.cart <- train(LetterGrade ~., data=record3, method = "rpart", metric = metric, trControl = control)
fit.knn <- train(Status ~., data=VS, method = "knn", metric = metric, trControl = control)
...
inTrain <- createDataPartition(y =VS$Status, p=0.7, list=FALSE)
training <- VS[inTrain,]
testing <- VS[-inTrain,]
...
# Train the model.
fit <- train(Status ~ ., data=training, method='glm')
...
resultsfit <- predict(fit, testing)
# Show accuracy on test set.
confusionMatrix(resultsfit, testing$Status)

# Train the model.
knn <- train(Status ~ ., data=training, method='knn')
...
knn
# Run model on test set.
resultsknn <- predict(knn, testing)
# Show accuracy on test set.
confusionMatrix(resultsknn, testing$Status)


# random forest
# Train the model.
rf <- train(Status ~ ., data=training, method='rf')
...
rf
# Run model on test set.
resultsrf <- predict(rf, testing)
# Show accuracy on test set.
confusionMatrix(resultsrf, testing$Status)
